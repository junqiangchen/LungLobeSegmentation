...
def _create_conv_net(X, image_z, image_height, image_width, image_channel, phase, drop, n_class=2):
    inputX = tf.reshape(X, [-1, image_z, image_height, image_width, image_channel])  # shape=(?, 32, 32, 1)
    # Vnet model
    # layer1->convolution
...
class Vnet3dModuleMultiLabel(object):
    ...
    def __init__(self, image_height, image_width, image_depth, channels=1, numclass=3,
        ...
        self.weight_loss = [1., 1., 1., 1., 1., 1., 1.]
        self.Y_pred = _create_conv_net(self.X, self.image_depth, self.image_height, self.image_width, self.channels,
                                       self.phase, self.drop, self.numclass)

        self.cost = self.__get_cost(self.Y_pred, self.Y_gt, costname[0], gamma=4)
    ...
    def __get_cost(self, Y_pred, Y_gt, cost_name, gamma=2):
        ...
        if cost_name == "categorical_dicePfocalloss":
            loss = categorical_dicePfocalloss(Y_pred, Y_gt, self.weight_loss, 100, 1., gamma)
        return loss
    ...
    def __loadnumtraindata(self, train_images, train_lanbels, num_sample, num_sample_index_in_epoch):
        ...
        for num in range(len(batch_xs_path)):
            ...
            subbatch_ys[num, :, :, :, :] = np.reshape(label,
                                                      (self.image_depth, self.image_height, self.image_width,
                                                       self.labelchannels))
        subbatch_xs = subbatch_xs.astype(np.float) / 255.
    ...
    def train(self, train_images, train_labels, model_path, logs_path, learning_rate,
        ...
        for i in range(train_epochs):
            ...
            if i % DISPLAY_STEP == 0 or (i + 1) == train_epochs:
                ...
                save_images(gt, showwind, path=logs_path + 'gt_%d_epoch.png' % (i), pixelvalue=42)

                result = np.reshape(pred_arg[0], (self.image_depth, self.image_height, self.image_width))
                result = result.astype(np.float)
                save_images(result, showwind, path=logs_path + 'predict_%d_epoch.png' % (i), pixelvalue=42)
